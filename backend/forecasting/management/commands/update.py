...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for f in Forecasts.objects.all().order_by("-created_at"):
            ...
            # Require minimum ForecastData for training; AgileData is optional
            if fd.count() < min_fd:
                fail = " <- Fail"
            else:
                fail = " <- Manual"
        ...
        if len(prices) == 0:
            logger.warning("No price data available, skipping GB60 processing")
            return
            ...
            if len(missing_fc) > 0:
                ...
                if len(fc) > 0:
                    ...
                    if len(ff) > 0:
                        ...
                        # Use left merge to keep all ForecastData records, even if price data is missing
                        train_X = train_X.merge(prices["day_ahead"], left_index=True, right_index=True, how="left")
                        
                        # Drop rows where we don't have price data (can't train without targets)
                        ...
                        # Handle cross-validation with limited samples
                        if len(train_X) > 0:
                            cv_folds = min(5, len(train_X))  # Use fewer folds if less samples
                            scores = cross_val_score(
                                xg_model, train_X, train_y, cv=cv_folds, scoring="neg_root_mean_squared_error"
                            )
                            logger.info(f"Cross-val score ({cv_folds} folds): {scores}")
                            
                            xg_model.fit(train_X, train_y, sample_weight=sample_weights, verbose=True)
                        else:
                            logger.warning("Empty training set - skipping model fitting")
                            scores = np.array([0.0])  # Default score for empty training data
                        ...
                        if len(train_X) > 0:
                            ...
                            results["pred"] = results["day_ahead"] * 0.5  # Use naive prediction if not trained
                        ...
                        # Only create/clear plots if model was trained
                        if len(train_X) > 0:
                    ...
                    # Only create forecast record if we have trained data (scores was defined)
                    if 'scores' in locals():
                        ...
                        logger.warning("No historical forecasts found in database. Need at least one forecast to train the model.")
